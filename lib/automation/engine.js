...
export async function executeWorkflow(definition, { input = {}, onNodeEvent = async () => {} } = {}) {
  validateWorkflowDefinition(definition);
  const context = { nodes: {}, input, workflow: { variables: definition.variables || {} }, execution: { startedAt: new Date().toISOString() } };
...
async function executeNode(node, input, context) {
  ...
  if (node.type === "crm.create") throw new AutomationError("NOT_CONFIGURED", "CRM node'u için workspace CRM tablo eşlemesi henüz yapılandırılmadı.", { httpStatus: 422 });
