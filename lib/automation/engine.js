...
export async function executeWorkflow(definition, { input = {}, workspaceId, onNodeEvent = async () => {} } = {}) {
  ...
  const deadline = Date.now() + Math.min(Math.max(Number(definition.settings?.maxExecutionTimeMs) || 300000, 1000), 300000);
  ...
  while (current) {
    if (Date.now() > deadline) throw new AutomationError("TIMEOUT", "Workflow maksimum çalışma süresini aştı.", { name: "TimeoutError", retryable: false, httpStatus: 504 });
    if (visited.has(current.id)) throw new AutomationError("WORKFLOW_CONFIG", "Döngü tespit edildi.", { httpStatus: 400 });
    visited.add(current.id); const started = Date.now(); const nodeExecutionId = await onNodeEvent({ node: current, status: "running" });
    try {
      const { output, retryCount } = await executeNodeWithRetry(current, incoming, context); context.nodes[current.id] = { output };
      const event = { nodeId: current.id, nodeName: current.name, nodeExecutionId, status: "succeeded", input: incoming, output: { ...output }, durationMs: Date.now() - started, retryCount }; logs.push(event); await onNodeEvent({ node: current, ...event });
      ...
      const normalized = normalizeAutomationError(error); const event = { nodeId: current.id, nodeName: current.name, nodeExecutionId, status: "failed", input: incoming, error: { code: normalized.code, message: normalized.message, retryable: normalized.retryable }, durationMs: Date.now() - started }; logs.push(event); await onNodeEvent({ node: current, ...event }); throw Object.assign(normalized, { executionLogs: logs });
    }
  }
  return { status: "succeeded", output: incoming, logs, startedAt: context.execution.startedAt, finishedAt: new Date().toISOString() };
}

async function executeNodeWithRetry(node, input, context) {
  const policy = node.retryPolicy || {};
  const maxAttempts = Math.min(Math.max(Number(policy.maxAttempts) || 1, 1), 5);
  let lastError;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try { return { output: await executeNode(node, input, context), retryCount: attempt - 1 }; }
    catch (error) {
      lastError = normalizeAutomationError(error);
      if (!lastError.retryable || attempt === maxAttempts) throw lastError;
      const delay = Math.min((Number(policy.initialDelayMs) || 500) * (policy.backoff === "exponential" ? 2 ** (attempt - 1) : 1), 5000);
